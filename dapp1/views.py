...
def index(request):
    ...
    return render(request, "index.html", {'role': request.session['role'],'subscription_status': subscription_status,'days': days})


def popup(request):
    return render(request,'Doctor/popup.html')
...
def pdf_view(request):
   if(request.method=="GET"):
    return render(request, 'Doctor/pdf_view.html')
        ...
        messages.success(request,"Thank You For Registeration Your 15 days Trial Plan is Activated  ")
...
def update_medicine(request,doctor_medicine_id):
    if(request.method=="GET"):
        if('doctor_id' in request.session):
            ...
            return HttpResponse("<h1>Please Login Again your session is Expired or Refresh Page</h1><br><a href='https://drishtis.app/DrishtisDesktop/login/'>login</a>")
    else:
        ...
        for appointment in data:
            if(appointment['consultation_id'] is not None):
                ...
                if(consult_data['patient_status']==2):
                    bill_res=requests.post('https://drishtis.app/drishti_medicalrecord/api/get_bill_header_details/',json={"consultation_id":appointment['consultation_id']})
    ...
    if(request.method=="GET"):
        ...
        if(vital_response.json().get('message_code')==1000):
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                print('1073 else,if')
                html = render_to_string('Doctor/initial_assesment.html', {"data1": data1, 'pain_scale_range': range(1, 11),'role':request.session['role']})
                return HttpResponse(html)
            else:
                print('1077 else else')
                return render(request, 'Doctor/initial_assesment.html', {"data1": data1, 'pain_scale_range': range(1, 11),'role':request.session['role']})
        ...
        if(request.session['role']=='Doctor'):
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                context = {
                    ...
                    "get_patient_boimterics_vitals": get_patient_boimterics_vitals,
            ...
            #             "medicine_instruction":medicine_instruction,'pain_scale_range': range(1, 11),
            #         })
        
        else:
            messages.success(request, 'Initial Assesment Added successfully!')
...
def update_initial_assesment(request):
    ...
    if(request.session['role']=='Doctor'):
        ...
        messages.success(request, 'Initial Assesment Updated successfully!')
        return redirect(get_all_doctor_appointments)
...
def Consultation(request,id):
    if(request.method == "GET"):
        ...
        if(get_patient_by_appointment_id.get('consultation_id')):
            ...
            print(consult_data)
            ###########Daycare medication###########
            daycaremedication_url="https://drishtis.app/drishti_medicalrecord/api/get_daycare_medication_details/"
            api_para={"consultation_id":request.session['consultation_id']}
            daycaremedic_response=requests.post(daycaremedication_url,json=api_para)
            if(daycaremedic_response.json().get('message_code')==1000):
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':  # Optional: Check if the request is via AJAX
                    print("1559 yes ajax")
    ...
    #######################################################################################################################
        ...
        return render(request, 'Doctor/consultation.html', {"get_patient_by_appointment_id":get_patient_by_appointment_id,
            ...
            # print(request.POST)
            #return HttpResponse("else part of consultation")       
########################## insert consultaions ########################################
            # Get_Patient_Boimterics_Vitals(requests,request.session['appointment_id'])
            ...
            if('consultation_id' in request.session):
                # return HttpResponse("1455 else if")
                update_consultation = {
                ...
                kco_str = ', '.join(kco_list) if kco_list else ''
                advice_str = ', '.join(advice_list) if advice_list else ''
...
#####################Update prescription details######################
                ...
                Get_Patient_By_Appointment_Id(requests,id)
                if(get_patient_by_appointment_id.get('consultation_id')):
                    request.session['consultation_id']=get_patient_by_appointment_id.get('consultation_id')
                    print("the consultation id is present",get_patient_by_appointment_id.get('consultation_id'))
                    consultation_url="https://drishtis.app/drishti_medicalrecord/api/get_consultation_byconsultationid/"
            ...
            #######################################################################################################################
                ...
                #return HttpResponse("1728 else if")
...
def get_pdf_link(request):
    ...
    # print(pdfurl)
    pdf_link=((response.json().get("message_data"))[0]).get("pdf_url")
...
def paid(request):
    ...
    print("new",new_patient_outstanding)
...
def clinic_pdf(request):
    try:
        ...
        if pdf_url:
            ...
            return render(request,'Doctor/clinicpdf.html',{'pdf_url':pdf_url})
        else:
        ...
        return render(request,'Doctor/prescriptionsetting.html',{'prescription_settings':prescription_settings,"timestamp": timestamp})
         ...
         # Extract form data from POST request
            ...
            if response.json().get('message_code') == 1000:
                ...
                return redirect(prescription_setting)
            else:
...
def patientselect(request,id): 
    if(request.session['subscription_status'] == 'active'):  
        ...
        return render(request,'Doctor/patientselect.html',{'data1':data1,'data2':data2,'role':request.session['role']})
    else:
        messages.success(request,"As your subscription has ended, you cannot proceed.")
        return HttpResponse(" ")
...
def add_member(request):
    ...
    if response.status_code == 200:
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                print('add member 2621 if')
                return render(request, 'Doctor/initial_assesment.html', {"data1": data2, 'pain_scale_range': range(1, 11),'role':request.session['role']})
        else:
            print('add member 2621 else')
            return render(request, 'Doctor/initial_assesment.html', {"data1": data2, 'pain_scale_range': range(1, 11),'role':request.session['role']})
...
def get_all_pharmacist(request):
    ...
    if(api_res.json().get('message_code')==1000):
        ...
        pharmacists=[]
        messages.success(request,'No Pharmacist is Approved..')
    ...
    if('doctor_id' in request.session):
        if(request.method=='GET'):
            ...
            if(request.POST.get('death_certificate_id')):
                ...
                res = requests.post('https://drishtis.app/drishti_masters/api/update_death_certificate_details/',json=data)
                #print(res.text)
            else:
                #print(request.POST.get('death_certificate_id'),"add")
                res = requests.post('https://drishtis.app/drishti_masters/api/insert_death_certificate_details/',json=data)
                print(res.text)
            # res={
            #     "message_code": 1000,
...
def generate_deathCertificate_pdf(request):
    if request.method == 'POST':
        ...
        res = requests.post('https://drishtis.app/drishti_masters/api/generate_death_certificate_pdf/',json={'death_certificate_id':death_certificate_id})
        print(res.text)
        if(res.json().get('message_code')==1000):
            pdf_url = f"https://drishtis.app/drishti_masters/static/death_certificates/{death_certificate_id}.pdf"
        else:
            pdf_url = ""
...
def medical_certificate(request):
    if('doctor_id' in request.session):
        if(request.method=='GET'):
            ...
            master_res = requests.get('https://drishtis.app/drishti_masters/api/get_all_certificate_masters/')
            ...
            if(request.POST.get('CertificateIssue_id')):
                ...
                res = requests.post('https://drishtis.app/drishti_masters/api/update_certificate_issue/',json=data)
                #print(res.text)
            else:
                print(request.POST.get('CertificateIssue_id'),"add")
                res = requests.post('https://drishtis.app/drishti_masters/api/insert_certificate_issue/',json=data)
                #print(res.text)
...
def generate_MedicalCertificate_pdf(request):
    if request.method == 'POST':
        ...
        # Simulate the generated PDF URL
        res = requests.post('https://drishtis.app/drishti_masters/api/generate_certificate_issue_pdf/',json={'CertificateIssue_id':CertificateIssue_id})
        print(res.text)
        if(res.json().get('message_code')==1000):
            pdf_url = f"https://drishtis.app/drishti_masters/static/MedicalCertificates/certificate_{CertificateIssue_id}.pdf"
        else:
            pdf_url = ""
...
def add_daycare(request):
    if(request.method=='GET'):
        ...
        if(get_patient_by_appointment_id.get('consultation_id')):
            ...
            patientmedic_url="https://drishtis.app/drishti_medicalrecord/api/get_daycare_medication_details/"
            api_para={"consultation_id":request.session['consultation_id']}
            patientmedic_response=requests.post(patientmedic_url,json=api_para)
            patientmedic_data=(patientmedic_response.json().get("message_data"))
            medic_list=[]
        ...
        ########################## insert consultaions ########################################
            ...
            if('consultation_id' in request.session):
                    ...
                    for name in unique_list:
                      for medicine in all_medicines:
                          if(name==medicine['medicine_name']):
                             med_id_list.append(medicine['doctor_medicine_id'])
                    # medication_url="http://127.0.0.1:8000/api/insert_patient_medications/"
                    daycaremedication_url="https://drishtis.app/drishti_medicalrecord/api/insert_daycare_medication/"
                    for i in range(len(unique_list)):
                            medication_data={
                                "doctor_id": request.session["doctor_id"],
                                "patient_id": patient_id,
                            ...
                            response=requests.post(daycaremedication_url,json=medication_data)
                            print(response.text)
                ...
                for key, value in previous_name.items():
                    if key not in common_name:
                        delete_medic.append(key)
                        deletemedic_url=" https://drishtis.app/drishti_medicalrecord/api/delete_daycare_medication/"
                        deletemedic_response=requests.post(deletemedic_url,json={"DaycareMedication_id":value})
                        print(deletemedic_response.text)

                daycaremedic_url="https://drishtis.app/drishti_medicalrecord/api/get_daycare_medication_details/"
                daycaremedic_para={"consultation_id":request.session['consultation_id']}
                daycaremedic_response=requests.post(daycaremedic_url,json=daycaremedic_para)
                ...
                if(len(patientmedic_data)!=len(daycaremedic_data) or  request.POST["Fess"]!=request.POST['old_daycarefees']):
                    billheader_res=requests.post('https://drishtis.app/drishti_medicalrecord/api/get_bill_header_details/',json={"consultation_id":request.session['consultation_id']})
                    if(billheader_res.json().get('message_code')==1000):
                        ...
                        updatebillheader_res = requests.post("https://drishtis.app/drishti_medicalrecord/api/update_bill_header/",json={"consultation_id":request.session['consultation_id'],"total_bill_amount":float(total_amount),"Total_bill_payable":float(new_payable_amount)})
                        print(updatebillheader_res.text)

                        #update patient outstanding
                        updatepatient_res = requests.post("https://drishtis.app/drishti_pateint/api/update_patient_by_id/",json={"patient_id":patient_id,"outstanding":new_patient_outstanding})
                        print(updatepatient_res.text)

                        #update patient charges
                        updatepatientCharges_res = requests.post("https://drishtis.app/drishti_medicalrecord/api/update_patient_charges/",json={"patient_id":patient_id,"doctor_id":request.session['doctor_id'],"charges_amount":total_amount,"new_outstanding":new_patient_outstanding})
...
#####################Update prescription details######################
    ...
    #############################Patient Medications##############################
                ...
                if medicine_list[0]:
                    # medication_url="http://127.0.0.1:8000/api/insert_patient_medications/"
                    medication_url="https://drishtis.app/drishti_medicalrecord/api/insert_daycare_medication/"
                    for i in range(len(medicine_list)):
                            medication_data={
                                "doctor_id": request.session["doctor_id"],
                                "location_id":request.session['location_id'],
...
#         daycaremedic_url = "https://drishtis.app/drishti_medicalrecord/api/get_daycare_medication_details/"
#         patientmedic_response = requests.post(daycaremedic_url, json=api_para)
#         patientmedic_data = patientmedic_response.json().get("message_data")

#         #fetch doctor and clinic details
#         doctor_url="https://drishtis.app/drishti_doctor/api/get_doctor_by_id/"
#         doctor_res=requests.post(doctor_url,json={"doctor_id":request.session['doctor_id']})
#         doctor=doctor_res.json().get("message_data",{})
#         print(doctor)

#         clinic_url="https://drishtis.app/drishti_doctor/api/get_all_doctor_location/"
#         clinic_res=requests.post(clinic_url,json={"doctor_location_id":request.session['location_id']})
...
def daycarebillcharges(request):
    try:
        ...
        # Fetch patient data
        patient_url = "https://drishtis.app/drishti_pateint/api/get_patient_byid/"
        patient_res = requests.post(patient_url, json={"patient_id": consult_data['patient_id']})
...
def daycare_finalize(request):
    ...
    # Fetch daycare medication data
    daycaremedic_url = "https://drishtis.app/drishti_medicalrecord/api/get_daycare_medication_details/"
    daycaremedic_response = requests.post(daycaremedic_url, json={"consultation_id":request.session['consultation_id']})
    ...
    print(bill_data)
    billresponse = requests.post('https://drishtis.app/drishti_medicalrecord/api/insert_bill_header/',json=bill_data)
...
def get_unpaid_bills(request):
    if('doctor_id' in request.session):
        patientbill_res=requests.post("https://drishtis.app/drishti_medicalrecord/api/get_active_bills/",json={"doctor_id":request.session['doctor_id']})
...
def daycarepayment(request):
    if(request.method == 'GET'):
        ...
        print(billHeaderId)
        billheader_res = requests.post("https://drishtis.app/drishti_medicalrecord/api/get_active_bills/",json={"billHeader_id":billHeaderId})
        ...
        print(bill_data)
        updatebillheader_res = requests.post("https://drishtis.app/drishti_medicalrecord/api/update_bill_header/",json=bill_data)
        print(updatebillheader_res.text)
        ...
        update_appstatus_url="https://drishtis.app/drishti_appointment/api/update_appointment_status"
        appstatus_response=requests.post(update_appstatus_url,json={"appointment_id":consult_data['appointment_id'],"appointment_status":4})
        print(appstatus_response.text)

        update_consultstatus_url="https://drishtis.app/drishti_medicalrecord/api/update_consultation_status/"
        consultstatus_response=requests.post(update_consultstatus_url,json={"consultation_id":request.POST['consultation_id'],"consultation_status":3})
        print(consultstatus_response.text)

        #update patient outstanding
        updatepatient_res = requests.post("https://drishtis.app/drishti_pateint/api/update_patient_by_id/",json={"patient_id":int(request.POST['patient_id']),"outstanding":new_patient_outstanding})
        print(updatepatient_res.text)
